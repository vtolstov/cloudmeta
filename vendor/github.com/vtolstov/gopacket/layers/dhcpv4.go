...
func (p *DHCPv4) String() string {
	...
	return fmt.Sprintf("op: %s, xid: %#x, ciaddr: %v, yiaddr: %v, siaddr: %v, giaddr: %v, chaddr: %s, options: %s", p.Operation, p.Xid, fmt.Sprintf("%s", net.ParseIP(fmt.Sprintf("%s", p.ClientIP))), fmt.Sprintf("%s", net.ParseIP(fmt.Sprintf("%s", p.YourIP))), fmt.Sprintf("%s", net.ParseIP(fmt.Sprintf("%s", p.ServerIP))), fmt.Sprintf("%s", net.ParseIP(fmt.Sprintf("%s", p.GatewayIP))), p.ClientHWAddr.String()[:17], options)
}
